...
// be isomorphic to P^1.

// This version does post-processing on genus 1, 2 and 3 models.
// This post-processing might be quite slow. The Klein quartic takes about 10 minutes.

// Input data is read from several files. One file enumerates subgroups
// and other files contains Fourier expansions of generators of the modular
// function field as well as a choice of conjugate.

// The assumption is that the subgroups of GL_2(Z/N Z) act on the left,
...
// This script takes two arguments: l and l2.
// l is a label for the group to compute and l2 is a label for the covering group.

if (not assigned l) or (not assigned l2) then
  printf "This script requires a command line argument.\n";
  printf "The usage is magma l:=label l2:=coverlabel model.m";
  quit;
else
  parse := Split(l,".");
...
// and H must literally be a subgroup of Hcover. X_Hcover must have
// genus zero, and we store a conjugate to use.
...
// Choose a covering group for which we actually have a stored Fourier expansion.
...
System("rm -f " cat logfilename);
SetLogFile(logfilename);

if coverlabel ne "1.1.0.1" and (not fileexists("modfunc" cat coverlabel cat ".txt")) then
  printf "I didn't find the Fourier expansion for a modular function for group %o.\n",coverlabel;
  bad := 0;
  bad2 := 1/bad;
end if;
...
printf "Building model for %o as a cover of %o.\n",l,coverlabel;
printf "The degree of that covering map is %o.\n",ind/indcov;
if (ind/indcov ge 12) and (gen ge 1) then
  printf "Degree >= 12 and genus >= 1.\n";
  printf "This script is probably the wrong choice. Try canmodel approach.\n";
  quit;
end if;
...
// Because this script was originally written for a right action,
// we transpose the generators of H and Hcover

gl2zhatind := Index([magma_gl2zhat[i][1] : i in [1..#magma_gl2zhat]],l);
genlist := Generators(magma_gl2zhat[gl2zhatind][2]);
H := sub<GL(2,Integers(N)) | [ Transpose(g) : g in genlist]>;

// We have stored the Fourier expansion of a generator for the function field.
...
if (coverlabel ne "1.1.0.1") then
  ...
  haupt := newhaupt;
  coverind := Index([ magma_gl2zhat[i][1] : i in [1..#magma_gl2zhat]],coverlabel);
  if (Ncover ne N) then
    Hcover := GL2Lift(Hcover,N);
  end if;
  genlist := [ g : g in Generators(Hcover)];
  Hcover := sub<GL(2,Integers(N)) | [Transpose(g) : g in genlist]>;
  chk, elt := IsConjugateSubgroup(GL(2,Integers(N)),Hcover,H);
  H := GL2MinimizeGenerators(Conjugate(H,elt));
else
  haupt := jInvariant(q);
...
// This routine returns the Fourier expansion of (9/Pi^2)*p((c*t+d)/N;t)
...
// The action is on the right, the convention that Shimura takes.
...
// Step 2 - Find a subgroup K of H so that X_K has more cusps than X_L
// for any subgroup L > K.

Hcovercusps := GL2CuspCount(Hcover);
indexbound := 1;

// The part could be revised to be made more efficient in the event
// that we have a more comprehensive subgroup list.
...
while (found eq false) do
  ...
  while done eq false do
    mo := MinimalOvergroups(Hcover,L[Lind]);
    overcusps := Max([ GL2CuspCount(mo[i]) : i in [1..#mo]]);
//  if (GL2DeterminantIndex(L[Lind]) eq 1) and (GL2CuspCount(L[Lind]) gt overcusps) then
//printf "For indexbound = %o, group %o, cusps = %o, overcusps = %o.\n",indexbound,Lind,GL2CuspCount(L[Lind]),overcusps;
if (GL2CuspCount(L[Lind]) gt overcusps) then
      found := true;
      done := true;
      subsub := L[Lind];
...
// First, find elements of the Eisenstein subspace that are fixed
// under the intersection of H with SL_2.
// The dimension of the space of Eisenstein series is the number of cusps - 1,
// and the relation is that the sum of all p((ct+d)/N) is zero.
...
for g in Generators(GL2MinimizeGenerators(sl2sub)) do
...
matlist := [];
genlist := SetToSequence(Generators(GL2MinimizeGenerators(subsub)));
for g in genlist do
...
// Enumerate the representatives of the left cosets of subsub in covergp 
// sorted by coset of sub in covergp

// S1 and S2 are sets of representatives for the left cosets
...
sympols := [];
while (done eq false) do
  if #sympols lt Min(it,#S2) then
    // Compute the it-th elemenetary symmetric polynomial.
    cursympols := [];
    subsetstomult := SetToSequence(Subsets({ j : j in [1..#S2]},it));
    for i in [1..#S1] do
      term := Parent(fourierlist[i][1])!0; 
      for t in [1..#subsetstomult] do
	printf "Computing symmetric polynomial %o. Poly %o/%o term %o/%o.\n",it,i,#S1,t,#subsetstomult;	      
	term := term + &*[ fourierlist[i][m] : m in subsetstomult[t]];
      end for;
      Append(~cursympols,term);
    end for;
    Append(~sympols,cursympols);
  end if;  
  // Are symmetric polynomials of degree = it enough??
  printf "Trying degree %o.\n",it;
  polstomake := RestrictedPartitions(it,{jj : jj in [1..#S2]});
  basispols := [];
  for i in [1..#S1] do
    lst := <>;
    for jj in [1..#polstomake] do
      Append(~lst,&*[ sympols[polstomake[jj][k]][i] : k in [1..#polstomake[jj]]]);
    end for;
    Append(~basispols,lst);
  end for;
  if #SequenceToSet(basispols) eq #S1 then
    done := true;
    wt := 2*it;
    printf "Symmetric polynomials of degree %o suffice.\n",it;
    printf "Effective weight is %o.\n",wt;
  else
    it := it + 1;
  end if;
end while;
...
while (done eq false) do
  ...
  while (done2 eq false) do
    seq := seqstotry[it];
    formsused := [ &+[ seq[m]*basispols[i][m] : m in [1..#polstomake]] : i in [1..#S1]];
    if #SequenceToSet(formsused) eq #S1 then
      done2 := true;
      done := true;
      printf "Using seq = %o with %o.\n",seq,polstomake;
    else
      it := it + 1;
      if (it gt #seqstotry) then
        done2 := true;
...
if (wt eq 2) then
  ...
  effwt := wt;
  if (effwt mod 4 eq 2) then
    e4pow := Floor((effwt-6)/4);
    denomfunc := Eisenstein(6,q : Precision := Ceiling(2*prec/N))*Eisenstein(4,q : Precision := Ceiling(2*prec/N))^e4pow;
  else
    e4pow := Floor(effwt/4);
    denomfunc := Eisenstein(4,q : Precision := Ceiling(2*prec/N))^e4pow;
  end if;
end if;
...
opttouse := 2;
if (gen eq 0) and (ind/indcov ge 14) then
  opttouse := 1;
end if;
...
printf "LLLing for real.\n";
L3, X3 := LLL(newlat);
newerbase := [];
printf "Changing coordinates.\n";
for i in [1..#newbase] do
  newerbase[i] := &+[ X3[i][j]*newbase[j] : j in [1..#newbase]];
  newerbase[i] := newerbase[i] - Trace(newerbase[i])/Degree(K);  
end for;
minpolys := [ MinimalPolynomial(newerbase[i]) : i in [1..#newerbase]];
rightdeg := [ j : j in [1..#newbase] | Degree(minpolys[j]) eq Degree(K) ];
// Believe it or not, sometimes every element of newerbase[i] lies in a proper subfield of K.
if (#rightdeg eq 0) then
  mx := 2;
  donegood := false;
  while (donegood eq false) do
    seqstotry := [ [] ];
    for j in [1..#newerbase] do
      cur := seqstotry;
      new := [];
      for a in cur do
        if (#a eq 0) then
    ...
    elts := [];
    minpolys := [];   
    for i in [1..#seqstotry] do
      elttry := &+[ seqstotry[i][m]*newerbase[m] : m in [1..#newerbase]];
      Append(~elts,elttry);
      Append(~minpolys,MinimalPolynomial(elttry));	 			   end for;
    rightdeg := [ j : j in [1..#minpolys] | Degree(minpolys[j]) eq Degree(K) ];
    if (#rightdeg gt 0) then
      donegood := true;
      newerbase := elts;
    else
      mx := mx + 1;
...
if (gen eq 1) then
  ...
  localpt := true;
  for p in PrimeFactors(2*3*5*7*11*N) do
    if IsPrime(p) then
      chk := IsLocallySolvable(C,p : Smooth := true);
      if not chk then
        printf "The curve has no local points at p = %o.\n",p;
	localpt := false;
      end if;    
    end if;
  end for;
  if (localpt eq false) then
    gen := 4;
    // OK. The genus isn't actually 4, but this will send it to the generic post-processing code.
  else
    findpt := false;
    bound := 10;
    done := false;
    while (done eq false) do
      ...
      for i in [1..#pts] do
        if not IsSingular(pts[i]) then
          ...
          plac := Places(pts[i]);
	  for j in [1..#plac] do
            if (Degree(plac[j]) eq 1) and (findpt eq false) then
              pt := plac[j];
	      findpt := true;
	      done := true;
            end if;
      ...
      if (findpt eq false) then
        bound := 2*bound;
	if (bound gt 60000) then
	  done := true;
	  gen := 4;
	  printf "It looks like this is a locally solvable genus 1 curve with no point!\n";
	end if;  
      end if;
    end while;
    if (findpt eq true) then
      EE, phiphi := EllipticCurve(C,pt);
      EE2, phiphi2 := MinimalModel(EE);
      printf "Minimal weierstrass model is %o.\n",EE2;      
      Cmpcover := map<C -> ProjectiveSpace(Rationals(),1) | [C.1,C.3]>;
      phiphi2inv := Inverse(phiphi2);
      printf "Inverting map to Weierstrass model. This might take a while.\n";
      timtim := Cputime();
      chk, phiphiinv := IsInvertible(phiphi);
      printf "Done. Time taken was %o sec.\n",Cputime(timtim);
      bigmap := phiphi2inv*phiphiinv*Cmpcover;
      ...
      modelfilename := "model" cat l cat ".txt";
      System("rm -f " cat modelfilename);
      PrintFile(modelfilename,Sprintf("X := EllipticCurve(%o);",aInvariants(EE2)));
      PrintFile(modelfilename,"return X;");
      // Write map to file
      mapfilename := l cat "map" cat coverlabel cat ".txt";
      System("rm -f " cat mapfilename);
      ...
      mapname := "map" cat Sprintf("%o_%o_%o_%oto%o_%o_%o_%o",lsplit[1],lsplit[2],lsplit[3],lsplit[4],coversplit[1],coversplit[2],coversplit[3],coversplit[4]);
      DD<x,y,z> := EE2;
      ades := AllDefiningPolynomials(finbigmap);
      ades2 := [ [ Evaluate(ades[i][1],[x,y,z]), Evaluate(ades[i][2],[x,y,z]) ] : i in [1..#ades]];
      PrintFile(mapfilename,mapname cat Sprintf(" := map<DD -> CC | %o>;",ades2));    
      PrintFile(mapfilename,"return " cat mapname cat ";");
      printf "Done with model computation.\n";
      quit;      
    end if;
  end if;  
end if;

if (gen eq 2) or (gen eq 3) then
  QQQQ<x,y> := PolynomialRing(Rationals(),2);
  newpoly := eval Sprintf("%o",niceplane);
  C := ProjectiveClosure(Curve(AffineSpace(Rationals(),2),newpoly));
  Cmpcover := map<C -> ProjectiveSpace(Rationals(),1) | [C.1,C.3]>;   
  chk := IsHyperelliptic(C);
  if chk then
    chk, B, mp := IsHyperelliptic(C);
    B2, mp2 := ReducedMinimalWeierstrassModel(B);
    printf "Minimal weierstrass model is %o.\n",B2;
    mp2inv := Inverse(mp2);
    chk, mpinv := IsInvertible(mp);
    ...
    modelfilename := "model" cat l cat ".txt";
    System("rm -f " cat modelfilename);
    R<xx> := PolynomialRing(Rationals());
    hp1, hp2 := HyperellipticPolynomials(B2);
    PrintFile(modelfilename,"R<xx> := PolynomialRing(Rationals());");
    PrintFile(modelfilename,Sprintf("X := HyperellipticCurve([%o,%o]);",hp1,hp2));
    PrintFile(modelfilename,"return X;"); 
  else
    mp := CanonicalMap(C);
    B := CanonicalImage(C,mp);
    mp := map<C -> B | DefiningPolynomials(mp)>;
    B2, mat := MinimizeReduce(B);
    ...
    PrintFile(modelfilename,Sprintf("X := Curve(P2,[ %o ]);",Evaluate(DefiningPolynomial(B2),[X,Y,Z])));
    PrintFile(modelfilename,"return X;");    
  end if;
  // Write map to file
  mapfilename := l cat "map" cat coverlabel cat ".txt";
...
if (gen eq 0) then
  ...
  // Way 2 - Use Magma to parametrize the genus zero curve.
    ...
    israt := true;
    if not GL2ContainsCC(H) then
      israt := false;
      printf "Modular curve fails to have real points.\n";
    end if;
    for prim in plist do
      chk := IsLocallySolvable(C,prim : Smooth := true);
      if (chk eq false) then
        printf "The curve is not locally solvable at p = %o.\n",prim;
        israt := false;
      end if;
    end for;    
    if israt eq false then
      QQQQ<x,y> := PolynomialRing(Rationals(),2);
      newpoly := eval Sprintf("%o",niceplane);
      C := ProjectiveClosure(Curve(AffineSpace(Rationals(),2),newpoly));
      // Write model to file
    ...
    // Old genus zero parametrization code
    bound := 10;
    found := false;
    while found eq false do
    ...
    A0 := Parametrization(C,P);
    printf "Found it!\n";
    A := ImproveParametrization(A0);
    xparam := A([x,1])[1];
    yparam := A([x,1])[2];
    printf "We have X = %o.\n",xparam;
    printf "We have Y = %o.\n",yparam;
    ...
    printf "Converting to simple curve equation. This may take some time.\n";
    timtim := Cputime();
    ffelt := Eltseq(newerbase[modelind]);
    yfourier := &+[ Evaluate(ffelt[i],haupt)*modf^(i-1) : i in [1..#ffelt]];
    printf "Done. Time taken was %o sec.\n",Cputime(timtim);
...
function field of %o.\n",coverlabel,l;
  ...
  PrintFile(fileout,Sprintf("KK<z> := CyclotomicField(%o);\n",N));
  PrintFile(fileout,"RRRRR<qq> := PuiseuxSeriesRing(KK);");
  genlist := [ Transpose(ggg) : ggg in GL2MinimizeGenerators(H)];
  PrintFile(fileout,Sprintf("H := sub<GL(2,Integers(%o)) | %o>;",N,[ Eltseq(genlist[i]) : i in [1..#genlist]]));
  expdenom := ExponentDenominator(newhaupt);
  bestexpdenom := LCM([ Denominator(a/expdenom) : a in [expdenom*Valuation(newhaupt)..expdenom*AbsolutePrecision(newhaupt)-1] | Coefficient(newhaupt,a/expdenom) ne 0]);
  endprec := Floor(bestexpdenom*AbsolutePrecision(newhaupt))/bestexpdenom;
  reparsedhaupt := BigO(qq^endprec);
  ...
  PrintFileMagma(fileout,reparsedhaupt);
  PrintFile(fileout,";");
  PrintFile(fileout,"return haupt, H;");
  printf "Fourier expansion of hauptmodul written to %o. Done with model computation.\n",fileout;
// Write model to file
  // Write model to file
  modelfilename := "model" cat l cat ".txt";
