// Here l2 is the label for a subgroup H that doesn't contain -I
// and l1 is the label for <H,-I>.
// First it computes a model for X_<H,-I>. Then it
// produces an equation for the universal elliptic curve over X_<H,-I>
// corresponding to the "correct" quadratic refinement.
...
for line in LinesOfInputFile do 
  ...
  gens0 := GetModularCurveGenerators(quadratic_refinements[1], generators);
...
varsnames := ["a","b","c","d","e","f","g","h","i","j","k","l","m","n","o","p"];
...
gens0 := GetModularCurveGenerators(l2);
...
// First, find a low degree divisor on C
...
printf "Genus of modular curve is %o. Using effective divisor of degree %o.\n",g,Degree(E);

M2 := CreateModularCurveRec(N2,gens0);
irregularcusps := not (&and M2`regular);
if irregularcusps then
...
totalprec := 0;
for c in [1..#chosencusps] do
  galoiscusp0 := GL2Galois meet Conjugate(gp,M`cusps[chosencusps[c]]);
  // The subfield of Q(zeta_N) corresponding to galoiscusp is the field of definition of the Fourier coeffs.
  galoiscusp := Sort([g[2][2] : g in galoiscusp0]);
  //printf "For cusp %o, Galois group is %o.\n",c,galoiscusp;
  KK := KKlist[c][1];
...
assert &and [ IsWeaklyZero(weakzero[i]) : i in [1..#chosencusps]];
//printf "The rational function is %o/%o.\n",fnum,fdenom;
