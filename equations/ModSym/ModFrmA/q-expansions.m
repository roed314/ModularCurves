...
function cusp_term(M, idx)  // ModFrmA, RngIntElt -> FldRatElt
   N := Level(M); 
   if N eq 1 then 
      cuspterm := 0;
...
intrinsic PrecisionBound(M::ModFrmA : Exact := false) -> RngIntElt
...
intrinsic NewformPrecisionBound(M::ModFrmA : Exact := true) -> RngIntElt
...
declare attributes ModFrmAElt : coeff_ring;

intrinsic CoefficientRing(f::ModFrmAElt : Bound := -1) -> Rng
...
a_n with n <= Bound.}
   if assigned f`coeff_ring then return f`coeff_ring; end if;
   if IsNewform(f) then
	 ...
	 return IntegerRing();
      ...
      O:=Order([Coefficient(qexp,n) : n in [2..bnd]]);
      f`coeff_ring:=O;
      return O;
   else
      f`coeff_ring:=Parent(Coefficient(f,1));
      return Parent(Coefficient(f,1));
   end if;   
end intrinsic;

intrinsic CoefficientField(f::ModFrmAElt) -> Fld
{The field in the which the Fourier coefficients lie.}
   return FieldOfFractions(Parent(Coefficient(f,1)));
end intrinsic;

intrinsic Coefficient(f::ModFrmAElt, n::RngIntElt) -> RngElt
{The nth Fourier coefficient of f.}
   requirege n, 0;
   return Coefficient(PowerSeriesInternal(f,n+1),n);
end intrinsic;

intrinsic '!'(R::RngSer, f::ModFrmAElt) -> RngSerElt
...
intrinsic Basis(M::ModFrmA, prec::RngIntElt) -> SeqEnum[RngSerPowElt]
{Same as qExpansionBasis}
   return qExpansionBasis(M, prec);
end intrinsic;

intrinsic qExpansionBasis(M::ModFrmA, prec::RngIntElt) -> SeqEnum[RngSerPowElt]
...
intrinsic qExpansion(f::ModFrmAElt, prec::RngIntElt) -> RngSerPowElt
{The q-expansion of the modular form f to absolute precision prec.}
   return PowerSeries(f,prec);
end intrinsic;

intrinsic qExpansion(f::ModFrmAElt) -> RngSerPowElt
{"} // "
   return PowerSeries(f);
end intrinsic;

intrinsic PowerSeries(f::ModFrmAElt) -> RngSerPowElt
{"} // "
   return PowerSeries(f,Precision(Parent(f)));
end intrinsic;

intrinsic PowerSeries(f::ModFrmAElt, prec::RngIntElt) -> RngSerPowElt
{"} // "
...
intrinsic Reductions(f::ModFrmAElt, p::RngIntElt) -> List
...
intrinsic pAdicEmbeddings(f::ModFrmAElt, p::RngIntElt) -> List
...
intrinsic ComplexEmbeddings(f::ModFrmAElt) -> List
...
/*
   ...
   eps : GrpDrchAElt,  a Dirichlet character (not necessarily primitive) 
...
intrinsic Bernoulli(k::RngIntElt, eps::GrpDrchAElt) -> RngElt
...
function Lvalue(chi, s : Precision := 100)
    ...
    prod := &*[1 - chi_0(p) * p^(-s) : p in primes];
    return prim_val * prod;
end function;
...
// Exact version giving the values divided by C_k = (-2pi i)^k / N^k (k-1)!
function QHurwitzZetaFunction(N, d, k)
    K := CyclotomicField(N);
    X := Elements(DirichletGroupFull(N));
...
function HurwitzZetaMuFunction(N, n_inv, k : Precision := 100)
    CC := ComplexField();
    Z_N_star := [i : i in [0..N-1] | GCD(i,N) eq 1];
    X := Elements(DirichletGroupFull(N));
    L_vals := Vector([CC!Lvalue(chi,k : Precision := Precision) : chi in X]);
    A := Matrix([[chi(m) : m in Z_N_star] : chi in X]);
    B_k := Transpose(Matrix([[CC!X[i](m)*L_vals[i]
			      : m in Z_N_star] : i in [1..#X]]));
    a_k := Rows((ChangeRing(A,CC)*B_k)^(-1))[1] * EulerPhi(N);
    n_inv_idx := Index(Z_N_star, n_inv mod N);
    return a_k[n_inv_idx];
end function;

// Computes H * C_k / 2
function QHurwitzZetaMuFunction(N, n_inv, k)
    K := CyclotomicField(N);
    Z_N_star := [i : i in [0..N-1] | GCD(i,N) eq 1];
    X := Elements(DirichletGroupFull(N));
    X := [chi : chi in X | chi(-1) eq (-1)^k];
    L_vals := Vector([QLvalue(chi,k) : chi in X]);
    A := Transpose(Matrix([[chi(m) : m in Z_N_star] : chi in X]));
    B_k := Matrix([[X[i](m)*L_vals[i]
			      : m in Z_N_star] : i in [1..#X]]);
    KK := BaseRing(B_k);
    a_k := Rows((ChangeRing(A, KK)*B_k)^(-1))[1] * EulerPhi(N);
    n_inv_idx := Index(Z_N_star, n_inv mod N);
    return K!a_k[n_inv_idx];
end function;

// This computes the function sigma_k^(c,d)(n) summing over all m | n such that
// n/m = c mod N the expression sign(m)*m^k*mu^(d*m)

function SigmaFunction(k, c, d, n, N)
    CC := ComplexField(); i := CC.1; pi := Pi(CC);
    mu := Exp(2*pi*i / N);
    s := 0;
    pos_divs := Divisors(n);
    neg_divs := [-x : x in Divisors(n)];
    divs := pos_divs cat neg_divs;
    good_divs := [m : m in divs | (n div m - c) mod N eq 0];
    for m in good_divs do
	s +:= Sign(m) * m^k * mu^(d*m);
    end for;
    return s;
end function;

// This computes the previous function for all n up to max_n, more efficiently.
function SigmaFunctionUpTo(k, c, d, max_n, N)
 //   CC := ComplexField(); i := CC.1; pi := Pi(CC);
    //   mu := Exp(2*pi*i / N);
    K := CyclotomicField(N);
    mu := K.1;
    s := AssociativeArray([1..max_n]);
    for n in [1..max_n] do
	s[n] := 0;
    end for;
    c_N := c mod N;
    d_N := d mod N;
    max_l := (max_n - c_N) div N;
    min_l := - ((max_n + c_N) div N);
    neg_range := [min_l..-1];
    if IsZero(c_N) then
	pos_range := [1..max_l];
    else
    ...
    for l in neg_range do
	l_cont := l*N + c_N;
	max_m := max_n div (-l_cont);
	for m in [-max_m..-1] do
	    n := m*l_cont;
	    s[n] -:= m^k * mu^(d_N*m);
	end for;
    end for;
    for l in pos_range do
	l_cont := l*N + c_N;
	max_m := max_n div l_cont;
	for m in [1..max_m] do
	    n := m*l_cont;
...
// Compute the q-expansion of the nonnormalized Eisenstein Series G_k^(c,d)
// This uses [DS] Theorem 4.2.3
function FullGammaEisensteinG(N, c, d, k, prec)
    // CC := ComplexField(); i := CC.1; pi := Pi(CC);
    // _<q> := PowerSeriesRing(CC);
    K := CyclotomicField(N);
    _<q> := PowerSeriesRing(K);
    if c mod N eq 0 then 
	// zeta := HurwitzZetaFunction(N, d, k);
	zeta := QHurwitzZetaFunction(N, d, k);
    else
	zeta := 0;
    end if;
    // C_k := (-2*pi*i)^k / Factorial(k-1);
    a := AssociativeArray([0..prec-1]);
    a[0] := zeta;
    // a[0] := zeta * N^k / C_k;
    s := SigmaFunctionUpTo(k-1, c, d, prec-1, N);
    for n in [1..prec-1] do
	a[n] := s[n];
    end for;
    f := &+[a[n]*q^n : n in [0..prec-1]];
    return f + O(q^prec);
end function;
...
function NumApprox(a,K : D := 10^6)
    ...
    if v[1] eq 0 then
	error Sprintf("bound %o is not large enough. Should be larger than %o!", D, (d+1)*a);
    end if;
    eps := a + t_C / v[1];
    a_K := -t / v[1];
    return a_K, Norm(eps);
...
// The approimation applied to a power series
function ApproximatePowerSeries(f, K, N)
    _<q> := PowerSeriesRing(K);
    prec := AbsolutePrecision(f);
    // Bound on the coefficients
    B := (Degree(K)+1)*Maximum([Abs(Coefficient(f, n)) : n in [0..prec-1]]);
    // Bound on the denominator - check!!!!
    // Using denominator of n-th Bernoulli number
    B *:= Denominator(BernoulliNumber(2*N));
    return &+[NumApprox(Coefficient(f, n),K : D := B)*q^n
	    : n in [0..prec-1]] + O(q^prec);
end function;

// Magma likes to normalize the q-expansions so that the first
// non-zero coefficient with positive index is 1.
// We do just that.
function PowerSeriesNormalizeMagma(f : eps := 10^(-20))
    // Here we proceed to normalize according to Magma's convention
    pivot := 1;
    is_exact := IsExact(Parent(Coefficient(f,pivot)));
    if is_exact then
	while IsZero(Coefficient(f, pivot)) do
	    pivot +:= 1;
	end while;
    else
	while Abs(Coefficient(f, pivot)) lt eps do
	    pivot +:= 1;
	end while;
    end if;
...
// At the moment this funciton is not used, as we may equally well
// use the non-normalized G_k^(c,d) and this resolves some of the
// difficulties of computing the rational values for the Hurwitz zeta mu function.
// Compute the normalized Eisenstein series E_k^(c,d)
function FullGammaEisenstein(N, c, d, k, prec)
    eps := (N gt 2) select 1 else 1/2;
    Z_N_star := [i : i in [0..N-1] | GCD(i,N) eq 1];
    // K := CyclotomicField(EulerPhi(N));
    // E := eps * &+[HurwitzZetaMuFunction(N, n, k) *
    E := eps * &+[QHurwitzZetaMuFunction(N, n, k) *
		  FullGammaEisensteinG(N, n*c, n*d, k, prec)  : n in Z_N_star];
    // E := PowerSeriesNormalizeMagma(E);
    // This is still complex. Returning to algebraic numbers
    // TODO : Change to the correct field in general
    // _<q> := PowerSeriesRing(K);
    // E := &+[Round(Re(Coefficient(E, n)))*q^n : n in [0..prec-1]] + O(q^prec);
    // Do we have a good bound on the denominator?
    /*
    E := &+[num_approx(Coefficient(E, n),K)*q^n
	    : n in [0..prec-1]] + O(q^prec);
    */
    return E;
end function;
...
intrinsic CyclotomicEmbedding(f::ModFrmAElt) -> Map
...
function BasisOfWeightOneCuspForms(M,prec)
   ...
   for i := 1 to #M`is_wt1_dihedral do 
      ...
      if M`is_wt1_dihedral[i][2] then 
        ...
        // F is a ModFrmAElt, vecs are vectors specifying combinations of the qExpansionBasis of MS
...
function q_expansion_basis_using_lower_weight(k, prec, spaces : dims:=[], dim_bound:=Infinity(),
   ...
   assert &and &cat[ [Type(f) eq ModFrmAElt : f in M] : M in spaces];
...
/*
 ...
 'type' is a string, and 'data' depends on the type. The options are:
    ...
    type = "elts_mult_elts", data = [*seq1,seq2*] :
           ...
           where seq1 and seq2 are sequences of ModFrmAElt's
    ...
    type = "low_weight_spaces", data = [M1,M2,...,Mn] :  
           ...
           where each Mi is a list of ModFrmAElt's generating a subspace 
           of some ModFrmA space with the same level as M (but lower weight).
