...
/****-*-magma-* EXPORT DATE: 2004-03-08 ************************************
   ...
   $Header: /home/was/magma/packages/ModFrmA/code/RCS/subspaces.m,v 1.6 2002/06/28 04:31:34 was Exp $
...
intrinsic CuspidalSubspace(M::ModFrmA) -> ModFrmA
...
intrinsic EisensteinSubspace(M::ModFrmA) -> ModFrmA
...
intrinsic NewSubspace(M::ModFrmA) -> ModFrmA
{The new subspace of M.}
   return NewSubspace(M, 0);
end intrinsic;

intrinsic NewSubspace(M::ModFrmA, p::RngIntElt) -> ModFrmA
...
intrinsic ZeroSubspace(M::ModFrmA)  -> ModFrmA
...
//   sub<M|fs> -> ModFrmA 
// where M::ModFrmA, fs::SeqEnum[ModFrmAElt] or ModFrmAElt or ModFrmA
// These spaces are not (necessarily) intended to have full functionality.
intrinsic SubConstr(M::ModFrmA, fs::. ) -> ModFrmA, .
{}
   // Note: not allowed to use require/error ...
   
   // make fs a sequence
   if Type(fs) eq ModFrmAElt then fs := [fs];  // so that sub<M|f> will work
   elif Type(fs) eq ModFrmA then fs := Basis(fs);
   ...
   S`basis := fs;  // this is quick and dirty!  The fs are ModFrmAElt's in M 
   return S, _;
end intrinsic;


intrinsic 'meet'(M1::ModFrmA, M2::ModFrmA) -> ModFrmA
