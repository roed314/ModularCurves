...
load "new_models.m";  // For the equations of modular curves.
...
searchDiv2:=function(X,bd,tf : max_hyperplanes := 10000);
	g:=Genus(X);
	//
	...
	ctr:=0;
	printf "Searching for points by intersecting with %o hyperplanes: ", #C;
	for a in C do
	  printf ".";
		ctr:=ctr+1;
		if ctr gt max_hyperplanes then
			break a;
		end if;
