...
MWSieveFiniteIndex := function(X, QuotientX, WMatrix, QuadraticPts, Fields, Generators, BasePoint, MWPrimes, M)
	...
	for p in MWPrimes do 
		...
		divsp := [NewReduce(X, Xp, genDiv) : genDiv in Generators]; // reductions of (generators of subgroup of MW group) mod p
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
		...
		poshat := {DD : DD in degr2 | pi((JFp!(psi(DD - bpp)))) in imGhat};  // Set S_{p,M} = {degree 2 divisors D of X/F_p with D - bp in im(G in J(F_p)/M)}
		...
		phi_pM := hom< A -> JFpmodM | [pi(JFp!psi(divp)) : divp in divsp] >; // The map phi_{p,M}. (A is isomorphic to the subgroup of J(Q) of finite index generated by divs Generators)
		Bp := Kernel(phi_pM);  
