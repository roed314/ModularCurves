...
MWSieveFiniteIndex := function(X, QuotientX, WMatrix, QuadraticPts, Fields, Generators, BasePoint, MWPrimes, M)
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
		...
		degree2divisors_mod_p := {1*place1 + 1*place2 : place1 in places_of_degree_1_mod_p, place2 in places_of_degree_1_mod_p}
								join {1*place : place in places_of_degree_2_mod_p}; 
		...
		image_A_in_JFpmodM := sub< JFpmodM | [modM(JFp!divisor_to_JFp(divisor_mod_p)) : divisor_mod_p in divisors_mod_p] >; // Image of A in J(F_p)/M
		// Set S_{p,M} = {degree 2 divisors D of X/F_p with D - bp in im(A in J(F_p)/M)}
		SpM := {divisor_mod_p : divisor_mod_p in degree2divisors_mod_p | modM((JFp!(divisor_to_JFp(divisor_mod_p - bpp)))) in image_A_in_JFpmodM};
		...
		// TpM is now T_{p,M}
		iotaTpM := {modM(JFp!(divisor_to_JFp(divisor_mod_p - bpp))) : divisor_mod_p in TpM};  // The set iota_{p,M}(T_{p,M}).

		// The map phi_{p,M}. (A is isomorphic to the subgroup of J(Q) of finite index generated by divs Generators)
		phi_pM := hom< A -> JFpmodM | [modM(JFp!divisor_to_JFp(divisor_mod_p)) : divisor_mod_p in divisors_mod_p] >;
