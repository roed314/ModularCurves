...
A := AbelianGroup([0,0,0]); // subgroup of J(Q) of finite index
...
MWSieveFiniteIndex := function(X, QuotientX, WMatrix, QuadraticPts, Fields, Generators, BasePoint, MWPrimes, M)
	...
	A := AbelianGroup([0 : i in [1..#Generators]]); // abstract free abelian group isomorphic to subgroup of J(Q) generated by Generators (TODO: allow torsion?)
	Ws := [**]; 
	Bs := [**];
	...
	for p in MWPrimes do 
		...
		//divisors_mod_p := [];    // Build up to list of generators for G reduced mod p
		Rks := [];      // Ranks of residue disc matrices
		...
		divisors_mod_p := [NewReduce(X, Xp, genDiv) : genDiv in Generators]; // reductions of (generators of subgroup of MW group) mod p
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
		...
		degree2divisors := {1*pl1 + 1*pl2 : pl1 in places_of_degree_1_mod_p, pl2 in places_of_degree_1_mod_p} join {1*pl : pl in places_of_degree_2_mod_p}; 
		...
		imGhat := sub< JFpmodM | [pi(JFp!psi(divp)) : divp in divsp] >; // Image of G in J(F_p)/M
		poshat := {DD : DD in degree2divisors | pi((JFp!(psi(DD - bpp)))) in imGhat};  // Set S_{p,M} = {degree 2 divisors D of X/F_p with D - bp in im(G in J(F_p)/M)}
		posP := {DD : DD in poshat | not DD in known_divisors_mod_p};   // Remove reductions of all known points,
		
		for i in [1..#QuadraticPts] do  // then add back in those that don't pass the Chabauty test
			if Rks[i] eq 0 then
				posP join:= {known_divisors_mod_p[i]};
			end if; 
		end for;
		
		// posP is now T_{p,M}
		jposP := {pi(JFp!(psi(DD - bpp))) : DD in posP};  // The set iota_{p,M}(T_{p,M}).

		phi_pM := hom< A -> JFpmodM | [pi(JFp!psi(divp)) : divp in divisors_mod_p] >; // The map phi_{p,M}. (A is isomorphic to the subgroup of J(Q) of finite index generated by divs Generators)
